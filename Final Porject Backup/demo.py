...
def main(screenshot=False):
    ...
    for i in range(THETA_DIVIDE_SIZE):
        path.append([1, 1, 2 * np.pi/THETA_DIVIDE_SIZE * (i+1)])
        path1.append([1, 1, 2 * np.pi/THETA_DIVIDE_SIZE * (i)])
    path2.append([1, 1, 0])
    path2.append([1, 1, np.pi/2])
    path2.append([1, 1, np.pi])
    path2.append([1, 1, -np.pi/2])

    # execute Path
    execute_trajectory(PR2, base_joints, path, sleep=0.1)
    wait_for_user()
    execute_trajectory(PR2, base_joints, path, sleep=0.5)
    wait_for_user()
    execute_trajectory(PR2, base_joints, path1, sleep=0.5)
    wait_for_user()
    execute_trajectory(PR2, base_joints, path2, sleep=0.5)
    print("=======================================")
